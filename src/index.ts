...
const MARKER = '<!-- funny-pr-image-bot -->';

function getEnv(name: string, required = true): string | undefined {
	const value = process.env[name];
	if (!value && required) {
		console.error(`[pr-funny-image] Missing required env: ${name}`);
		process.exit(1);
	}
	return value;
}
...
async function main() {
	...
	if (!repoArg) {
		console.error('[pr-funny-image] Missing repo. Pass --repo owner/repo or set GITHUB_REPOSITORY.');
		process.exit(1);
	}
	...
	if (!prNumber) {
		console.error('[pr-funny-image] Missing PR number. Pass --pr or ensure this runs on a pull_request event.');
		process.exit(1);
	}
	...
	if (existing) {
		console.log('[pr-funny-image] Comment already exists. Skipping.');
		return;
	}
	...
	const body = [
		...
		'',
		'<sub>Generated by pr-funny-image-bot using Vercel AI SDK and OpenAI.</sub>'
	].join('\n');

	await postJson(`${apiBase}/issues/${prNumber}/comments`, githubToken, { body });
	console.log('[pr-funny-image] Comment posted.');
}

main().catch((err) => {
	console.error('[pr-funny-image] Failed:', err?.message || err);
	process.exit(1);
});